# 第九章 范型
`Golang` 1.18 已经开始默认支持范型了。通过范型可以写出支持多种类型的多态函数。支持一下特性：

- 函数可以有更多的类型参数，如 `func function[T any] (param K){}`
- 函数定义中的类型参数可以在函数体中使用
- 自定义类型也可以有类型参数， 如`type M[T any]{ value T}`
- 每个类型参数都需要有一个类型约束，比如`func F[T constraint](param T){}`
- 类型约束必须是`interface`类型
- 范型函数，只能被用在满足类型约束的类型上
- 使用范型函数和范型类型时，需要指定类型列表中的类型；
- 大部分时间，可以不用指定类型列表中的类型，编译器会做指定判断
...
可以看出：
- 相同类型的范型函数是不一样的，类似于每次新建都都新建了一个对象？
- 可以显示指定类型列表；
